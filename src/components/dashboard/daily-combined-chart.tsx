...
function DailyTooltip({ active, payload, label, avg }: any) {
...
interface DailyCombinedChartProps {
  data: {
    totalRecords: number;
    grandTotal: number;
...
export function DailyCombinedChart({ data }: DailyCombinedChartProps) {
  if (!data) {
    return (
      <Card>
  ...
  const activeDays = data.dailyData.filter((d) => Number(d.bultos) > 0).length;
  const avgQuantity = data.grandTotal > 0 ? Math.round(data.grandTotal / activeDays) : 0;
  const totalMissions = data.shifts.reduce((sum, s) => sum + s.total, 0);
  const tooltipContent = <DailyTooltip avg={avgQuantity} />;
